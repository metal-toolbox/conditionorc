...
func TestServerConditionList(t *testing.T) {
	var firmwareInstall rctypes.Kind = "firmwareInstall"

	var Inventory rctypes.Kind = "Inventory"
	...
	testcases := []struct {
		...
		},
		{
			...
			func(r *store.MockRepository) {
				r.EXPECT().
					List(gomock.Any(), gomock.Eq(serverID), gomock.Eq(rctypes.Pending)).
					Times(1).
					Return(conditions, nil)
			},
			func(t *testing.T) *http.Request {
				url := fmt.Sprintf("/api/v1/servers/%s/state/%s", serverID.String(), rctypes.Pending)
